...
export class ApiInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        let userLanguage = sessionStorage.getItem('ng2-webstorage|current_language');
        if (userLanguage != null) {
            userLanguage = userLanguage.replace(/\"/gi, '');
            clonedRequest = request.clone({ setHeaders: { 'Accept-Language': userLanguage.substring(0, 2) } });
        } else {
            if (this.configService.config != null) {
                const defLang = this.configService.config.DefaultLanguage.substring(0, 2);
        ...
        return next.handle(clonedRequest).pipe(
            ...
            catchError(err => {
                ...
                this.toastr.error(err.message, 'Error: ' + err.status + ' ' + err.statusText);
